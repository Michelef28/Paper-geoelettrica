...
frame = plt.gca()
ax.plot(bin_ohm, val_ohm, label='CCR', color='black', linestyle='dashed', linewidth=5)
ax.plot(bin_ert, val_ert, label='GCR', color='black', linestyle='dotted', linewidth=5) 
ax.plot(bin_j, val_j, label='Joint', color='black', linestyle='solid', linewidth=5)  
...
plt.savefig(r'Figure5.png')

# Salva la colorbar come immagine separata
fig_colorbar, ax_colorbar = plt.subplots(figsize=(8, 1))
cbar = mpl.colorbar.ColorbarBase(ax_colorbar, cmap=colmap2, orientation='horizontal')
cbar.set_ticks([0, 4, 10, 16, 26, 31, 40, 52, 68, 88, 114, 148, 192, 250])
cbar.ax.tick_params(labelsize=16)
cbar.set_label('Resistivity [Ω⋅m]', fontsize=18)
